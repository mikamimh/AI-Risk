...
VARIABLE_DICTIONARY: List[Dict[str, str]] = [
    ...
    {"variable": "Race", "definition": "Self-reported race/ethnicity", "origin": "Preoperative", "unit": "categorical", "transformation": "Not used as a model feature. Retained in the analytical dataset for STS Score input mapping (raceblack / raceasian fields), cohort description, fairness/subgroup analyses, and audit exports.", "in_model": "No (excluded from AI Risk predictive model — severe class imbalance at n=454: 435 White, 16 African American, 3 Asian, 0 non-White events; ablation confirmed TargetEncoder estimates are statistically unstable and ethically inappropriate as predictors; retained for STS mapping, audit, and fairness)", "domain": "Demographics"},
    ...
    {"variable": "Previous surgery", "definition": "History of prior cardiac surgery. Literal 'None' means no prior cardiac surgery. Training data: free-text for redo cases; individual inference form absence is canonicalized to None. Five audit-only columns are derived via parse_previous_surgery() — not model features.", "origin": "Preoperative", "unit": "None/free-text redo history", "transformation": "Categorical (TargetEncoder); audit columns: previous_surgery_any, previous_surgery_count_est, previous_surgery_has_combined, previous_surgery_has_repeat_marker, previous_surgery_has_year_marker", "in_model": "Yes", "domain": "Comorbidities"},
    ...
    {"variable": "Aortic Root Abscess", "definition": "Echocardiographic evidence of aortic root abscess; literal 'None' means absence of abscess", "origin": "Pre-Echocardiogram", "unit": "None/Yes/location text", "transformation": "Categorical (TargetEncoder)", "in_model": "No (not retained in current model)", "domain": "Echocardiographic"},
    ...
    {"variable": "Preoperative Medications", "definition": "List of preoperative medications; literal 'None' means no relevant medication in the source list", "origin": "Preoperative", "unit": "None/text medication list", "transformation": "Used for STS medication mapping only", "in_model": "No (STS input only)", "domain": "Medication"},
