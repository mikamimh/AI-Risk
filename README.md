...
| 5 | **Temporal Validation** | Apply the frozen trained model to an independently uploaded cohort. Layout: **Locked Model** → **Cohort Integrity** (upload, chronological check, normalization) → **Drift Summary** (training vs validation comparison, auto-interpretation, calibration drift warning) → **AI Risk — Primary Validation** (KPI cards: AUC, AUPRC, Brier, calibration intercept/slope, CIL, ICI, sensitivity, NPV, flag rate) → **Discrimination & Calibration table** → **Pairwise comparison** → **Risk categories** → **Interactive Graphs** → **Interpretation Summary** → **Threshold Analysis** (compact operating characteristics table with bundle-stored primary threshold highlighted, Youden labeled post-hoc; full TP/FP/TN/FN table in expander) → **Exploratory Local Recalibration** (structured all-methods table, guardrails) → case-level predictions → exports. |
...
- **Temporal Validation** exports `ai_risk_temporal_{version}_{date}_summary.xlsx`, `_predictions.csv`, `_report.pdf`, and `_report.md`. The summary workbook includes cohort summary, performance (with CIL, ICI, AUPRC baseline, calibration CI, flag rate), pairwise comparison, risk categories, calibration, case-level predictions, common-cohort results when applicable, normalization summary when applicable, and exploratory recalibration/threshold sheets when available. The PDF/Markdown report includes a Drift Summary (training vs validation), Calibration Drift Warning (when conditions are met), primary performance tables, and an Interpretation Summary.
...
- Binary clinical variables (11 columns: Left Main Stenosis ≥ 50%, Proximal LAD Stenosis ≥ 70%, CCS4, Hypertension, Dyslipidemia, PVD, Alcohol, Dialysis, Chronic Lung Disease, Critical preoperative state, Poor mobility): converted directly to 0/1 numeric before the preprocessor, bypassing TargetEncoder. This eliminates encoding noise for variables that carry only 1 bit of information. Validated by ablation V2 over 20 seeds (ΔAUC +0.007 vs baseline, p=0.005 paired Wilcoxon). Multi-level categorical variables (Diabetes, CVA, IE, Cancer ≤ 5 yrs, Anticoagulation, Pneumonia, Family Hx of CAD) are intentionally kept on TargetEncoder to preserve their clinical categories — a regression test (`tests/test_binary_encoding_guard.py`) locks this policy in place. See `docs/decisions/ADR-002-categorical-encoding.md` for the full encoding rationale (to be added in a follow-up PR).
...
- The primary threshold is taken from the bundle's `threshold_policy` (sensitivity-constrained 90% policy); legacy bundles without `threshold_policy` fall back to the fixed 8%
...
- Performance metrics are reported at the bundle-stored primary threshold
...
### Visual organization of the Temporal Validation tab

The tab is organized into six logical blocks, clearly separated from the exploratory sections:

1. **Dataset & Cohort Integrity** — total patients, events, event rate, completeness categories, STS eligibility, normalization warnings.
2. **Drift Summary** — training vs validation side-by-side comparison table (n, events, event rate, heavily imputed rate, prevalence ratio, absolute difference) with automatic interpretation of prevalence shift.
3. **Primary Frozen Performance** — AI Risk KPI cards (AUC, AUPRC, AUPRC baseline, Brier, calibration intercept, calibration slope, CIL, ICI, sensitivity, specificity, PPV, NPV, flag rate at locked threshold). Captioned: "Primary results use the locked frozen model without retraining, recalibration, or threshold adjustment." Followed by a **Calibration Drift Warning** (yellow box) when any of the following are triggered: validation/training event rate ratio < 0.5× or > 1.5×, |CIL| > 0.05, flag rate > 50%, or |calibration intercept| > 1.0.
4. **Threshold Operating Characteristics** — a compact table (Score, Threshold, Type, Sensitivity, Specificity, PPV, NPV, Flag Rate %, Event Rate Above, Event Rate Below) with the bundle-stored primary threshold row labeled "★ Primary" and Youden labeled "Post-hoc / exploratory". A separate expander holds the complete table with TP, FP, TN, FN for all models. Primary threshold interpretation text is shown automatically.
5. **Exploratory Local Recalibration** — in a clearly separated, visually distinct expander titled "Exploratory Local Recalibration — not primary performance". Shows a structured table with all scores × all methods (intercept-only, intercept+slope, isotonic), Brier before/after, calibration intercept/slope before/after, and "Recommended Use". Guardrails show reliability warnings if events < 50 (unreliable), < 100 (unstable for intercept+slope), or if isotonic is selected (always exploratory). The locked model and threshold are not altered.
6. **Exports** — XLSX, PDF, Markdown, CSV all include the new fields. PDF/Markdown report now contains a Drift Summary section, Calibration Drift Warning, primary performance tables, and an Interpretation Summary. The bundle-stored primary threshold is distinguished from Youden throughout.
...
- The **bundle-stored primary threshold** (sensitivity-constrained 90% policy, or legacy 8% for bundles without `threshold_policy`) remains unchanged for all primary classification metrics.
- Comparisons with EuroSCORE II and STS use the same cohort and the same outcome definition.
- Metrics reported: AUC (with 95% CI), AUPRC (with 95% CI), AUPRC baseline, Brier score, Calibration-in-the-Large (CIL), Integrated Calibration Index (ICI), calibration intercept and slope (with CI), Hosmer–Lemeshow p, sensitivity, specificity, PPV, NPV, flag rate (at the locked threshold), pairwise DeLong AUC comparisons, and risk-category distributions.
- **Calibration Drift Warning** — an automatic alert appears when any of the following conditions are detected: validation/training event rate ratio < 0.5× or > 1.5×, |CIL| > 0.05, flag rate > 50%, or |calibration intercept| > 1.0. This does not alter the primary results; it informs interpretation.
- **CIL and ICI** — CIL (Calibration-in-the-Large = mean predicted − mean observed) reflects overall bias; positive values indicate overestimation. ICI (Integrated Calibration Index via isotonic regression) is a continuous calibration measure; lower is better (0 = perfect).
- **Flag rate** — the proportion of the cohort predicted above the locked threshold; above 50% indicates a sensitivity-oriented rule-out operating point driven by the lower prevalence of the validation cohort.

### Exploratory analysis

An **Exploratory Local Recalibration** section is visually separated from the primary results and clearly labeled "not primary performance". It includes:

- **Post-hoc recalibration** — intercept-only, intercept + slope (logistic), and isotonic regression, applied to the validation cohort after the fact. Displayed as a structured table (Score × Method × Brier before/after × calibration intercept/slope before/after × Recommended Use).
- **Reliability guardrails** — warnings shown when events < 50 (unreliable estimates), < 100 (isotonic unstable), or when isotonic is selected (always flagged as exploratory-only with overfit risk note).
- **Threshold comparison** — fixed clinical thresholds (2%, 5%, 8%, 10%) and the **Youden's J optimum** per model. The bundle-stored primary threshold is labeled "★ Primary"; Youden is labeled "Post-hoc / exploratory". The fixed 8% is a historical comparator.
- **Event Rate Above / Below threshold** — shown alongside Sensitivity, Specificity, PPV, NPV, and Flag Rate % in the compact threshold operating characteristics table.

> **Important:** The exploratory section does not alter the primary results. Recalibration is post-hoc and data-driven; Youden's J is optimised on the same cohort being evaluated. Neither should be used to report primary model performance or to replace the locked threshold in clinical decision-making. The model bundle is not modified.
...
- Post-hoc calibration is applied inside each outer CV fold using a per-model strategy (see the AI Risk section above for the exact method per model); the inner calibration CV uses StratifiedKFold and does not accept a `groups` parameter. In the training cohort used for bundle v15 (Dataset_2025.xlsx), each patient contributes a single surgery (n_patients = n_surgeries = 454), so grouped and non-grouped cross-validation produce identical partitions; the "grouping not enforced in inner CV" note is a true structural limitation of `CalibratedClassifierCV` but has no operational effect here. The limitation is documented for transparency and for future cohorts where patients may contribute multiple surgeries.
...
- **Optimism bias in OOF leaderboard metrics.** The out-of-fold (OOF) AUC, AUPRC, Brier, and calibration metrics reported in the leaderboard are computed on the same cross-validation partition that the best-model selection algorithm uses (AUC ranking + clinical-usability guardrails + calibration-aware tiebreaker). Because the winning model is chosen on the same signal used to evaluate it, the reported OOF performance is an optimistic estimate of the true out-of-sample performance. This is the standard "model development" bias documented in Steyerberg (Clinical Prediction Models, 2nd ed., 2019) and the TRIPOD-AI reporting guideline. The magnitude is typically modest with multiple candidates and cross-validation, but it is not zero.
