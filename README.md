...
4. **Threshold Operating Characteristics** — a compact table (Score, Threshold, Type, Sensitivity, Specificity, PPV, NPV, Flag Rate %, Event Rate Above, Event Rate Below) with the 8% locked row labeled "★ Locked — Primary" and Youden labeled "Post-hoc / exploratory". A separate expander holds the complete table with TP, FP, TN, FN for all models. Locked threshold interpretation text is shown automatically.
...
6. **Exports** — XLSX, PDF, Markdown, CSV all include the new fields. PDF/Markdown report now contains a Drift Summary section, Calibration Drift Warning, primary performance tables, and an Interpretation Summary. The 8% locked threshold is distinguished from Youden throughout.
...
- **Threshold comparison** — fixed clinical thresholds (2%, 5%, 8%, 10%) and the **Youden's J optimum** per model. The 8% locked threshold is labeled "★ Locked — Primary"; Youden is labeled "Post-hoc / exploratory".
